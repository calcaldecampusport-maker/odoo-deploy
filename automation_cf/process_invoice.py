...
def find_or_create_supplier(rpc, company, data):
    ...
    if vat:
        for v in (vat, "ES" + vat):
            r = rpc.search_read("res.partner", [("vat", "=", v)], ["id"], limit=1, company_id=cid)
            if r:
                pid = r[0]["id"]
                break
    if not pid and name:
...
_BAD_TAX = ("intracomunitar", "importaci", "extracomunitar", "inversión", "no deducible",
            "special", "especial",
            # nomenclatura CORTA del l10n_es en inglés (BDs de CARAJFAM y BEST TRAINING):
            # '21% EU S' = intracomunitario, '21% EX G' = importación, '21% RC' = inversión
            # del sujeto pasivo, '21% ND' = no deducible, '21% IG' = bienes de inversión.
            "intra-community", "import", "reverse charge", "not deductible", "investment goods")
...
def find_purchase_tax(rpc, cid, rate):
    ...
    taxes = rpc.search_read("account.tax",
                            ...
                            ["id", "name", "description", "price_include"], company_id=cid)
    if not taxes:
        return None

    def score(t):
        nombre = (t.get("name") or "").strip().lower()
        # la descripción del l10n_es dice explícitamente el régimen ('(goods)',
        # 'intra-community (services)', 'purchase reverse charge'…)
        n = f'{nombre} {(t.get("description") or "").lower()}'
        s = 0
        ...
        if "corrientes" in n:
            s += 2
        # '21% G' (bienes) y '21% S' (servicios) son los ordinarios de estas BDs
        if re.fullmatch(r"\d+(?:[.,]\d+)?\s*%\s*[gs]", nombre):
            s += 6
...
def find_reverse_tax(rpc, cid, rate, kind=None):
    ...
    patrones = {"intracomunitario": ["intracomunitari"],
                "extracomunitario": ["extracomunitari"],
                "isp_nacional": ["inversión del sujeto pasivo", "inversion del sujeto pasivo", "ISP"]}
    for pat in patrones.get(kind, ["intracomunitari"]):
        taxes = rpc.search_read("account.tax",
                                [("type_tax_use", "=", "purchase"), ("amount", "=", rate),
                                 ("amount_type", "=", "percent"), ("company_id", "=", cid),
                                 ("name", "ilike", pat)],
...
def process(rpc, company, data, pdf_bytes=None, pdf_name=None):
    ...
    if dup:
        return {"status": "duplicate", "move_id": dup, "partner_id": partner_id}
