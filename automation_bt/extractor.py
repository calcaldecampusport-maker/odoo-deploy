...
- company_recipient (string or null): which GROUP COMPANY the document is ADDRESSED to — look at the
  CUSTOMER block ("Bill to" / "Receptor de factura" / "Cliente"), NEVER at the supplier:
    "BT"   = BEST TRAINING RINCON DE LA VICTORIA, S.L. (CIF/VAT ESB72349137, Spain)
...
- expense_account (string or null): ONLY if a reviewer rule or the reviewer note explicitly fixes the
  expense account code for this document (e.g. "627100"). Otherwise null. If a rule dictates the
...
def validate(data: dict) -> str | None:
    ...
    # supplier_vat NO es obligatorio: las facturas USA (CORP) no llevan NIF y el
    ...
    try:
        ...
        if dt == "nomina":
            especie = round(float((data.get("extra") or {}).get("salario_especie_total") or 0), 2)
