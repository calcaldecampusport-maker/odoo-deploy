...
LOW_CONFIDENCE = 0.7
_CLAVES = {"wiemspro_sl": 1, "wiemspro_corp": 3, "bio_sensors": 9}
...
def cargar_reglas(company, upload_id=None):
    ...
    import os
    import sqlite3
    db = os.path.join(os.getenv("WIEMSPRO_BACKEND", "/opt/carajfam-contab/backend"), "data", "app.db")
    try:
...
def main():
    ...
    log.info(f"[{company['name']}] extrayendo {p.name} (upload {args.web_upload_id}, {len(reglas)} reglas)")
    try:
        data = extractor.extract(p, company, hint=args.hint, rules=reglas)
    ...
    if not args.skip_po:
        try:
            ...
            if vat:
                for v in (vat, "ES" + vat):
                    r = rpc.search_read("res.partner", [("vat", "=", v)], ["id"],
                                        limit=1, company_id=cid_po)
                    if r:
                        pid_po = r[0]["id"]
                        break
