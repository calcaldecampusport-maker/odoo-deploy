...
ENV_FILE = "/etc/automation_austral.env"
...
def _fetch_today(env, target_date: date) -> dict:
    ...
    try:
        ...
        run_file = _Path(f"/tmp/extractor_runs_austral/{target_date.isoformat()}.json")
        if run_file.exists():
...
<p>Fecha: <b>{target_date.isoformat()}</b></p>"""]
    ...
    # === Bloque liquidaciones PS B2C del día ===
    import glob, json as _json
    from pathlib import Path as _Path
    ps_logs = sorted(glob.glob('/tmp/ps_liquidacion_*.json'), reverse=True)
    ps_recent = []
    for pl in ps_logs[:3]:
        try:
            d = _json.loads(_Path(pl).read_text())
            ps_recent.append({'file': pl, **d})
        except Exception:
            pass
    if ps_recent:
        parts.append("<h2>💳 Liquidaciones PS B2C contabilizadas</h2>")
        for runinfo in ps_recent:
            fechas = runinfo.get('fechas') or []
            asientos = runinfo.get('asientos') or []
            if not asientos: continue
            parts.append(f"<p>Fechas procesadas: <b>{', '.join(fechas)}</b></p>")
            parts.append("<table><thead><tr><th>Tipo</th><th>Ref</th><th>Asiento Odoo</th>"
                         "<th class='num'>Nº fac/abo</th><th class='num'>Base €</th>"
                         "<th>Detalle IVA</th><th class='num'>Total €</th><th>Estado</th></tr></thead><tbody>")
            for a in asientos:
                if a.get('skip'):
                    estado = f"YA EXISTE (id={a.get('existing_id')})"
                    parts.append(f"<tr><td>{a.get('tipo','-')}</td><td>{a.get('ref')}</td>"
                                 f"<td>{a.get('name','')}</td><td class='num'>—</td>"
                                 f"<td class='num'>—</td><td>—</td><td class='num'>—</td>"
                                 f"<td class='note'>{estado}</td></tr>")
                elif a.get('error'):
                    parts.append(f"<tr><td>{a.get('tipo','-')}</td><td>{a.get('ref')}</td>"
                                 f"<td>—</td><td class='num'>—</td><td class='num'>—</td>"
                                 f"<td>—</td><td class='num'>—</td><td class='note'>ERROR: {a.get('error')}</td></tr>")
                else:
                    iva_d = a.get('iva_detail') or {}
                    iva_str = '; '.join(f"{k}={v:.2f}" for k,v in iva_d.items()) if iva_d else '—'
                    parts.append(f"<tr><td>{a.get('tipo','-')}</td><td>{a.get('ref')}</td>"
                                 f"<td>{a.get('name')}</td><td class='num'>{a.get('n',0)}</td>"
                                 f"<td class='num'>{a.get('base',0):.2f}</td>"
                                 f"<td>{iva_str}</td><td class='num'>{a.get('total',0):.2f}</td>"
                                 f"<td>OK</td></tr>")
            parts.append("</tbody></table>")
...
def main():
    ...
    dudas_dir = Path("/tmp/dudas_austral")
