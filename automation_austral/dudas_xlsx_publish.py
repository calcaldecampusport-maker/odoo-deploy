#!/usr/bin/env python3
"""
Reads /tmp/dudas_austral/<vat>_dudas.json (produced by dudas_xlsx_collect.py),
...
Runs in /opt/automation_austral/venv (has google + openpyxl).
"""
...
INPUT_DIR = Path("/tmp/dudas_austral")
...
def publish(svc, payload: dict) -> dict:
    ...
    try:
        ...
        per_file = _Path(f"/tmp/periodic_austral/{payload['vat']}_periodic.json")
        if per_file.exists():
    ...
    try:
        ...
        for offset in range(0, 4):
            ...
            run_file = _Path(f"/tmp/extractor_runs_austral/{d.isoformat()}.json")
